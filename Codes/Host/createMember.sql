...
BEGIN
	...
	SELECT memberId,memberPassword into x,y FROM (Select memberId,memberPassword from MEMBER UNION SELECT memberId,memberPassword from MEMBER2) where memberId=(Select max(memberId) from (Select memberid from member2 union select memberid from member));
